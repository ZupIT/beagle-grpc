...
private val REQUEST_HTTP_DATA = RequestData(REQUEST_HTTP_URL)
private const val REQUEST_GRPC_URL =
    "http://grpc.test/grpc/someEndpoint?param=paramValue&param2=paramValue2"
private val REQUEST_GRPC_DATA = RequestData(REQUEST_GRPC_URL)
...
class GrpcClientTest {
    ...
    inner class ExecuteNonGrpcCallTest {
        ...
        fun testNullHttpClientCall() = runBlockingTest {
            ...
            val onSuccess: (ResponseData) -> Unit = {}
            val onError: (ResponseData) -> Unit =
                { responseData -> errorResponseData = responseData }
    ...
    inner class ExecuteGrpcCallTest {
        ...
        fun testEmptyScreenNameCall() = runBlockingTest {
            val requestData = RequestData(GRPC_ADDRESS)
            var errorResponseData: ResponseData? = null
            val onSuccess: (ResponseData) -> Unit = {}
            val onError: (ResponseData) -> Unit =
                { responseData -> errorResponseData = responseData }
        ...
        fun testAddRequestHeaders() = runBlockingTest {
            ...
            val requestData = RequestData(
                REQUEST_GRPC_URL,
                HttpAdditionalData(headers = mapOf(headerKey to expectedHeaderValue))
            )
        ...
        fun testRequestHeadersInterceptor() = runBlockingTest {
            val requestData = RequestData(REQUEST_GRPC_URL)
            val interceptorSlot = slot<HeaderClientInterceptor>()
            every {
                anyConstructed<ScreenServiceGrpcKt.ScreenServiceCoroutineStub>().withInterceptors(
                    capture(interceptorSlot)
                )
            } answers {
                this.callOriginal()
            }
        ...
        fun testSendCorrectRequestMessage() = runBlockingTest {
            ...
            coEvery {
                anyConstructed<ScreenServiceGrpcKt.ScreenServiceCoroutineStub>().getScreen(
                    capture(messageSlot)
                )
        ...
        fun testSendCorrectParamsForPost() = runBlockingTest {
            ...
            val request = RequestData(
                REQUEST_GRPC_URL,
                HttpAdditionalData(method = HttpMethod.POST, body = requestBody)
            )
            val responseMock: Messages.ViewNode = mockk(relaxed = true)
            val messageSlot = slot<Messages.ScreenRequest>()
            coEvery {
        ...
        fun testServerErrorCall() = runBlockingTest {
            ...
            val serverExceptionStatusMock =
                Status.ABORTED.withDescription(serverExceptionDescription)
        ...
        fun testServerSuccessCall() = runBlockingTest {
            ...
            val onSuccess: (ResponseData) -> Unit =
                { responseData -> successResponseData = responseData }
            val onError: (ResponseData) -> Unit = { }
